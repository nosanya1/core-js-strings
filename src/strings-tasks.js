...
function countVowels(str) {
  ...
  for (let i = 0; i < arrayFromString.length; i += 1) {
    if (vowels.includes(arrayFromString[i])) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  const strToLowerCase = str.replace(/[^\w]/g, '').toLowerCase();
  return strToLowerCase.split('').reverse().join('') === strToLowerCase;
}
...
function findLongestWord(sentence) {
  ...
  let longestWord = sentenceToArray[0];

  for (let i = 1; i < sentenceToArray.length; i += 1) {
    if (sentenceToArray[i].length > longestWord.length) {
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (c) => cipher[orig.indexOf(c)]);
}
...
function getCardId(value) {
  ...
  const suits = '♣♦♥♠';
  return (
    ranks.indexOf(value.at(0)) + suits.indexOf(value.at(-1)) * ranks.length
  );
}
